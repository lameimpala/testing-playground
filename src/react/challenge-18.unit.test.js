...
describe('Todos', () => {
    ...
    it('should add a todo item', async () => {
        ...
        fireEvent.change(getByLabelText('New Todo:'), {target: { value: 'My new todo'}});
    ...
    it('should clear out the input when a todo is added', async () => {
                // arrange
        // act
        const { getByLabelText, getByText, queryByDisplayValue } = render(<Todos />);
        fireEvent.change(getByLabelText('New Todo:'), {target: { value: 'My new todo'}});
        fireEvent.click(getByText('Add Todo'));
        await waitForElement(() => getByText('My new todo'));
    ...
    it('should add multiple todo items', async () => {
        ...
        fireEvent.change(getByLabelText('New Todo:'), {target: { value: 'My new todo2'}});
        fireEvent.click(getByText('Add Todo'));
        await waitForElement(() => getByText('My new todo2'));

        fireEvent.change(getByLabelText('New Todo:'), {target: { value: 'My new todo3'}});
        fireEvent.click(getByText('Add Todo'));
